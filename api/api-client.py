...
class APIClient:
    ...
    def create_master_products_batch(self,business_account_id: str, products: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
        ...
        try:
            response = self._make_request(
                ...
                data={'businessAccountId': business_account_id,
                      'products': products}
            )
            ...
            if response.get('success'):
                ...
                self.context.log.warning(f"Master product creation had issues: {response}")
