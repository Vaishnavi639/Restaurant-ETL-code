...
class APIClient:
    ...
    def __init__(self, base_url: str = None,context=None):
        ...
        self.session.headers.update({
            ...
            'x-access-token': 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6IjAxOTk5NTVhLWFjYmYtNzAwMC04MDAwLWIyMzBhMThlZTU3MSIsImZpcnN0TmFtZSI6Iktlc2F2YW4iLCJlbWFpbCI6ImtleXVyLmdldGFmdG9AZ21haWwuY29tIiwiaWF0IjoxNzU5MTU5NDYyfQ.7hhz3z2wjZ51Bb8nE5QaZcnewCf4oEMZM3rZ1RPBhdU'
        })
    ...
    # Add this method to the APIClient class (around line 500+)
    ...
    def create_whatsapp_template(
        ...
        """
        Create a WhatsApp template with sections
        
        Args:
            business_account_id: Business account ID
            template_data: Template structure with template info and sections
            
        Returns:
            API response with template ID
        """
        try:
            # Clean base URL to avoid double /v1
            base_url = self.base_url.rstrip('/').replace('/v1', '')
            endpoint = f"{base_url}/v1/channel-whatsapp/templates"
            
            # Get tokens from session headers
            access_token = self.session.headers.get('x-access-token')
            external_token = self.session.headers.get('x-external-token')
            ...
            logger.info(f"Creating WhatsApp template: {template_data['template']['name']}")
            ...
            logger.info(f"✓ Template created successfully: {result.get('id')}")
            return {
                'success': True,
                'data': result
            }
            ...
            logger.error(f"HTTP error creating template: {e.response.status_code} - {e.response.text}")
            return {
                'success': False,
    ...
    def get_products_by_business(
        ...
        """
        Fetch all products for a business account
        ...
        try:
            # Clean base URL to avoid double /v1
            base_url = self.base_url.rstrip('/').replace('/v1', '')
            endpoint = f"{base_url}/v1/products/"
            
            # Get token from session headers (since self.token doesn't exist)
            token = self.session.headers.get('x-external-token')
            ...
            logger.info(f"Fetching products for business: {business_account_id}")
            ...
            while True:
                ...
                if current_page >= total_pages or len(products) < limit:
                    break
            ...
            if all_products:
                logger.debug(f"Sample product keys: {list(all_products[0].keys())}")
            
            return all_products
            
        except requests.exceptions.HTTPError as e:
            logger.error(f"HTTP error fetching products: {e.response.status_code}")
