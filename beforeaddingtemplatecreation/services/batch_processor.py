...
class ETLBatchProcessor:
    ...
    def __init__(self, batch_id: int, business_account_id=None, context=None):
        ...
        # 🔹 Fetch business details ONCE and store as instance variables
        self.business_name = None
        self.business_industry_type = None
        self._fetch_business_details()
    ...
    def _fetch_business_details(self):
        """
        Fetch business details ONCE and store as instance variables
        Sets:
            - self.business_name: Name of the business
            - self.business_industry_type: Industry type (e.g., 'restaurant', 'grocery')
        """
        try:
            ...
            self.business_name = business_details.get('name', 'Unknown Business')
            
            industry_type_id = business_details.get("industry_type")
            self.business_industry_type = resolve_industry_type(industry_type_id)
            
            self.context.log.info(f"Business details loaded:")
            self.context.log.info(f"  - Name: {self.business_name}")
            self.context.log.info(f"  - Industry Type: {self.business_industry_type}")
            
        except Exception as e:
            self.context.log.warning(f"Failed to fetch business details, using defaults: {e}")
            self.business_name = 'Unknown Business'
            self.business_industry_type = 'grocery'  # Default to grocery
    ...
    def process_batch(self, mdb_products: List[MDBProduct]) -> BatchProcessingResult:
        ...
        """
        flow_type = f"🍽️ {self.business_industry_type.upper()}"
        ...
        try:
            ...
            if not self.business_industry_type == 'restaurant':
    ...
    def _stage_0_precheck(self):
        """
        Stage 0: Precheck if product already exists for business  update it with new data otherwise create product as full pipeline
        """
        self.context.log.info(f"\n{'=' * 80}")
        self.context.log.info(f"STAGE 0: Precheck")
        self.context.log.info(f"{'=' * 80}")
        product_to_update = []
        new_mdb_products = []
        ...
        for product in self.mdb_products:
            try:
                if product.article_id == None:
                    self.context.log.info(f"Product {product.product_name} does not have article id")
                    continue
                result_data = self.api_client.check_product_exists_by_article_id(business_account_id=self.business_account_id,article_id=product.article_id.strip())
                self.context.log.info(f"Checking if product exists for business {self.business_account_id} with article id {product.article_id}")
                
                self.context.log.info(f"Got data {'=' * 80}")
                if result_data.get('exists'):
                        self.context.log.info(f"Product already exists for business {self.business_account_id} with article id {product.article_id}")
                        self.context.log.info(f"Updating product {result_data.get('retailer_id')} with new data")
                        self.context.log.info(f"{'=' * 80}")

                        # Start with required fields
                        update_params = {
                            'retailer_id': result_data.get('retailer_id')
                        }

                        update_needed = False

                        # Check each field and add to params if changed
                        # if result_data.get('name') != product.product_name:
                        #     update_params['name'] = product.product_name
                        #     update_needed = True
                        try:
                            if float(result_data.get('price')) != float(product.price):
                                self.context.log.info(f"Price mismatch as json price is {product.price} and data price is {result_data.get('price')}")
                                update_params['price'] = product.price
                                update_needed = True
                        except (ValueError, TypeError):
                                self.context.log.info("type conversion error")  
                        self.context.log.info(f"after price check  update needed is {update_needed}")
                        # if result_data.get('quantity') != product.quantity:
                        #     update_params['quantity'] = product.quantity
                        #     update_needed = True

                        # Handle tax logic
                        tax_percentage = result_data.get('tax_percentage')
                        if tax_percentage is not None:
                            try:
                                # Convert to float for comparison
                                tax_percentage_float = float(tax_percentage)
                                is_tax = tax_percentage_float > 0.0
                            except (ValueError, TypeError):
                        ...
                        if product.is_tax != is_tax:
                            update_params['tax_slab'] = TAX_SLAB_TRUE if product.is_tax else TAX_SLAB_FALSE
                            update_needed = True

                        # Only call update if needed
                        if update_needed:
                            self.context.log.info("calling update API")
                            update_result = self.api_client.update_product(**update_params)
                            time.sleep(2)
                            if update_result.get('success'):
                                self.context.log.info(f"Product {result_data.get('retailer_id')} updated successfully")
                                # self.result.success_rate += 1
                                # self.result.success_count += 1
                            else:
                                self.context.log.info(f"Failed to update product {result_data.get('retailer_id')}: {update_result.get('message')}")
                                self.result.errors.append({
                                    'stage': 'precheck',
                    ...
                    new_mdb_products.append(product)
            except Exception as e:
                self.context.log.info(f"In exception Failed to check if product exists for business {self.business_account_id} with article id {product.article_id}: {str(e)}")
                self.context.log.info(f"{traceback.format_exc()}")
                self.result.errors.append({
                    'stage': 'precheck',
                    'error': str(e),
                    'type': 'warning'
        ...
        self.mdb_products = new_mdb_products

    def _stage_1_upc_matching(self):
        """
        Stage 1: Match products by UPC code against master products
        """
        self.context.log.info(f"\n{'=' * 80}")
        ...
        products_with_upc = []
        for product in self.mdb_products:
            self.context.log.debug(f"Processing product: {product}")
            if product.article_id is not None or product.article_id != '':
                try:
                    if self.barcode_validotor.is_valid(product.article_id):
                        product.original_article_id = product.article_id
                        products_with_upc.append((product, product.article_id))
                    else:
                        product.original_article_id = product.article_id
                        product.article_id = None
                        self.products_to_generate.append(product)
                except Exception as e:
                    self.context.log.info(f"Issue validating barcode {product.article_id}: {str(e)}")
                    product.original_article_id = product.article_id
                    product.article_id = None
                    self.products_to_generate.append(product)
            else:
                self.context.log.info(f"Product {product} has empty article_id cannot be processed")
                continue
            self.context.log.debug(f"Processed product: {product}")

        # if not products_with_upc:
        #     self.context.log.info("No products with valid UPC codes")
        #     self.products_to_generate = self.mdb_products.copy()
        #     return

        self.context.log.info(f"Found {len(products_with_upc)} products with UPC codes")

        try:
            # Batch UPC lookup
            upc_codes = [upc for _, upc in products_with_upc]
            master_products = self.api_client.search_master_products_by_upc_batch(upc_codes)

            # Create mapping
            upc_to_master = {mp['upc_code']: mp for mp in master_products if mp.get('upc_code')}

            # Process matches
            for mdb_product, normalized_upc in products_with_upc:
                master_product = upc_to_master.get(normalized_upc)
                
                if master_product:
                    # CRITICAL: Use original description as name, not master product name
                    processed_product = self._create_processed_product_from_master(
                        mdb_product=mdb_product,
                        master_product=master_product,
                        match_type='upc'
                    )
                    self.upc_matched_products.append(processed_product)
                    self.context.log.info(f"UPC matched: {mdb_product.product_name} -> {master_product['afto_product_id']}")
                else:
                    self.products_to_generate.append(mdb_product)

            self.result.upc_matched_count = len(self.upc_matched_products)
            self.result.upc_matched_products = self.upc_matched_products

            self.context.log.info(f"UPC matching complete:")
            self.context.log.info(f"  - Matched: {len(self.upc_matched_products)}")
            self.context.log.info(f"  - Unmatched: {len(self.products_to_generate)}")

        except Exception as e:
            self.context.log.error(f"Error in UPC matching: {str(e)}", exc_info=True)
            self.products_to_generate = self.mdb_products.copy()
            self.result.errors.append({
                'stage': 'upc_matching',
                'error': str(e)
    ...
    def _stage_3_generate_content(self):
        ...
        for mdb_product in complete_products:
            ...
            if self.business_industry_type == 'restaurant' and not brand_name:
        ...
        for mdb_product, generated in zip(incomplete_products, generated_content_list):
            try:
                ...
                if self.business_industry_type == 'restaurant':
                ...
                processed = ProcessedProduct(
                    ...
                    brand_name=self._get_restaurant_brand_name() if self.business_industry_type == 'restaurant' else "Generic",
    ...
    def _get_restaurant_brand_name(self) -> str:
        ...
        Uses stored business_name instead of fetching again.
        """
        return self.business_name if self.business_name else 'House Special'

    def _stage_4_search_images(self):
        """
        Stage 4: Search for product images and upload to Azure Blob Storage
        FIXED: Generates stable product IDs for restaurants (no more None values)
        """
        import time
        import hashlib
        ...
        # Filter products that need images (ALL products without image_url)
        products_needing_images = [
            p for p in self.generated_products 
            if not hasattr(p, 'image_url') or not p.image_url
        ]

        if not products_needing_images:
            self.context.log.info("No products need image search (all products already have images)")
            return

        self.context.log.info(f"Found {len(products_needing_images)} products without images")
        self.context.log.info(f"Using confidence threshold: 60/100")
        self.context.log.info(f"Industry type: {self.business_industry_type}")
        
        # Log breakdown by match_type
        match_type_counts = {}
        for p in products_needing_images:
            match_type = getattr(p, 'match_type', 'unknown')
            match_type_counts[match_type] = match_type_counts.get(match_type, 0) + 1
        
        self.context.log.info(f"Breakdown by type: {dict(match_type_counts)}")
        ...
        for i, product in enumerate(products_needing_images):
            try:
                ...
                article_id = str(product.article_id) if hasattr(product, 'article_id') and product.article_id else None
                match_type = str(product.match_type) if hasattr(product, 'match_type') else 'unknown'
                ...
                # Generate stable, meaningful product_id for all cases
                if article_id and article_id != 'None' and article_id.strip():
                    # Case 1: Grocery products with valid UPC/Article ID
                    stable_product_id = article_id.strip()
                    self.context.log.info(f"  Using article_id: {stable_product_id}")
                    
                elif self.business_industry_type == 'restaurant':
                    # Case 2: Restaurant products - use hash of product name for uniqueness
                    clean_name_for_hash = product_name.strip().lower().replace(' ', '_')
                    name_hash = hashlib.md5(clean_name_for_hash.encode('utf-8')).hexdigest()[:12]
                    stable_product_id = f"rest_{name_hash}"
                    self.context.log.info(f"  Generated restaurant ID: {stable_product_id}")
                    
                else:
                    # Case 3: Generic products without UPC - use row index if available
                    if hasattr(product, 'original_row_index') and product.original_row_index is not None:
                        stable_product_id = f"gen_row_{product.original_row_index}"
                    else:
                        # Last resort: timestamp-based unique ID
                        timestamp_id = int(time.time() * 1000)
                        stable_product_id = f"gen_{i}_{timestamp_id}"
                    self.context.log.info(f"  Generated generic ID: {stable_product_id}")

                self.context.log.info(f"  Product type: {match_type}")
                self.context.log.info(f"  Brand: {brand_name or 'N/A'}")
                self.context.log.info(f"  Category: {category_name or 'N/A'}")
                self.context.log.info(f"  Final product_id: {stable_product_id}")
                ...
                try:
                    image_url = self.image_service.search_product_image(
                        ...
                        upc_code=article_id if article_id and article_id != 'None' else None,
                        business_industry_type=self.business_industry_type  
                    )
                ...
                # Rate limiting
                if i < len(products_needing_images) - 1:
                    delay = 0.5
                    time.sleep(delay)
                ...
                self.result.errors.append({
                    ...
                    'match_type': getattr(product, 'match_type', 'unknown'),
        ...
        self.context.log.info(f"  Success rate: {(successful_uploads/len(products_needing_images)*100):.1f}%")
    ...
    def _stage_5_create_master_products(self):
        ...
        try:
            ...
            master_products_data = [{
                ...
                'upc': normalize_upc_code(p.article_id) if not self.business_industry_type == 'restaurant' and p.article_id and self.barcode_validotor.is_valid_barcode(p.article_id) else None
            ...
            if response.get('success'):
                successful = response.get('data', {}).get('successful', [])
    ...
    def _stage_5a_index_master_products(self):
        ...
        Retries failed indexing with minimal required fields (afto_product_id, name)
        """
        self.context.log.info(f"\n{'=' * 80}")
        self.context.log.info(f"STAGE 5a: Index Master Products to Elasticsearch")
        self.context.log.info(f"{'=' * 80}")

        if not self.es_client:
            logger.warning("Elasticsearch client not available, skipping master product indexing")
            return

        if not self.master_products_created:
            self.context.log.info("No master products to index")
            return

        self.context.log.info(f"Indexing {len(self.master_products_created)} master products")

        try:
            es_documents = []
            
            for success_item in self.master_products_created:
                idx = success_item['index']
                
                # Extract product data from the API response structure
                master_product = success_item['product']
                
                # Try to find the corresponding generated product for vector embedding
                vector_embedding = None
                if idx < len(self.generated_products):
                    product = self.generated_products[idx]
                    if hasattr(product, 'vector_embedding'):
                        vector_embedding = product.vector_embedding
                
                # Build ES document - all data comes from the API response
                es_doc = {
                    'afto_product_id': master_product['afto_product_id'],
                    'product_name': master_product['name'],
                    'brand_name': master_product['brand']['name'],
                    'category': master_product['category']['name'],
                    'subcategory': master_product['subcategory']['name'],
                    'upc_code': master_product.get('upc_code'),
                    'vector_embedding': vector_embedding
                }
                
                es_documents.append(es_doc)
                self.context.log.info(f"✓ Prepared ES document [{idx}]: {master_product['afto_product_id']} - {master_product['name']}")

            self.context.log.info(f"\nPrepared {len(es_documents)} documents for Elasticsearch indexing")
            
            if es_documents:
                # First attempt: index with full documents
                result = self._bulk_index_to_elasticsearch(
                    index_name=ES_MASTER_PRODUCTS_INDEX,
                    documents=es_documents
                )

                if result.get('success'):
                    indexed_count = result.get('indexed_count', 0)
                    self.context.log.info(f"✓ Successfully indexed {indexed_count}/{len(es_documents)} master products to Elasticsearch")
                    
                    # Check if there were any partial failures
                    failed_count = len(es_documents) - indexed_count
                    if failed_count > 0:
                        logger.warning(f"⚠ {failed_count} documents failed to index, attempting retry with minimal fields...")
                        self._retry_failed_indexing(es_documents, result)
                else:
                    # All failed, retry with minimal fields
                    self.context.log.error(f"✗ Initial indexing failed: {result.get('error', 'Unknown error')}")
                    self.context.log.info(f"Retrying with minimal required fields (afto_product_id, product_name)...")
                    self._retry_failed_indexing(es_documents, result)
            else:
                logger.warning("No documents prepared for indexing")

        except Exception as e:
            self.context.log.error(f"✗ Exception in master product indexing: {str(e)}", exc_info=True)
            self.result.errors.append({
                'stage': 'master_product_indexing',
                'error': str(e),
                'exception_type': type(e).__name__
            })

    def _retry_failed_indexing(self, original_documents, initial_result):
        """
        Retry indexing failed documents with minimal required fields
        Only includes afto_product_id and product_name
        """
        try:
            # Get failed document indices if available
            failed_docs = initial_result.get('failed_items', [])
            
            # If we don't have specific failed items, retry all
            if not failed_docs:
                docs_to_retry = original_documents
            else:
                # Extract indices of failed documents
                failed_indices = [item.get('index') for item in failed_docs if 'index' in item]
                docs_to_retry = [original_documents[i] for i in failed_indices if i < len(original_documents)]
            
            if not docs_to_retry:
                self.context.log.info("No documents to retry")
                return
            
            self.context.log.info(f"Retrying {len(docs_to_retry)} documents with minimal fields")
            
            # Create minimal documents with only required fields
            minimal_documents = []
            for doc in docs_to_retry:
                minimal_doc = {
                    'afto_product_id': doc['afto_product_id'],  # Required field
                    'product_name': doc['product_name']  # Required field
                }
                minimal_documents.append(minimal_doc)
            
            # Retry with minimal documents
            retry_result = self._bulk_index_to_elasticsearch(
                index_name=ES_MASTER_PRODUCTS_INDEX,
                documents=minimal_documents
            )
            
            if retry_result.get('success'):
                retry_indexed = retry_result.get('indexed_count', 0)
                self.context.log.info(f"✓ Retry successful: indexed {retry_indexed}/{len(minimal_documents)} documents with minimal fields")
            else:
                # Even retry failed - log but continue
                self.context.log.error(f"✗ Retry also failed: {retry_result.get('error', 'Unknown error')}")
                self.context.log.error(f"Failed documents:")
                for doc in minimal_documents:
                    self.context.log.error(f"  - {doc['afto_product_id']}: {doc['product_name']}")
                
                # Store retry errors
                self.result.errors.append({
                    'stage': 'master_product_indexing_retry',
                    'error': retry_result.get('error'),
                    'failed_products': [
                        {
                            'afto_product_id': doc['afto_product_id'],
            ...
            self.context.log.error(f"✗ Exception during retry indexing: {str(e)}", exc_info=True)
            self.result.errors.append({
                'stage': 'master_product_indexing_retry',
                'error': str(e),
                'exception_type': type(e).__name__
