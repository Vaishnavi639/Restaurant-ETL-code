...
class ETLBatchProcessor:
    ...
    def process_batch(self, mdb_products: List[MDBProduct]) -> BatchProcessingResult:
        ...
        try:
            # Stage 0: Precheck - KEEP for both (uses article_id for grocery, name for restaurant)
            ...
            # Stage 2: Similarity Search - KEEP for both
            ...
            # Stage 6: Create final products - KEEP for both
    ...
    def _stage_0_precheck(self):
        """
        Stage 0: Precheck if product already exists for business
        - GROCERY: Check by article_id
        - RESTAURANT: Check by name
        """
        self.context.log.info(f"\n{'=' * 80}")
        self.context.log.info(f"STAGE 0: Precheck ({'Restaurant - by name' if self.is_restaurant else 'Grocery - by article_id'})")
        self.context.log.info(f"{'=' * 80}")
        ...
        for product in self.mdb_products:
            try:
                # Determine check field based on business type
                if self.is_restaurant:
                    check_field = product.product_name
                    if not check_field:
                        self.context.log.info(f"Product does not have name, skipping")
                        continue
                    self.context.log.info(f"Checking if product exists by name: {check_field}")
                    result_data = self.api_client.check_product_exists_by_name(
                        business_account_id=self.business_account_id,
                        name=check_field.strip()
                    )
                else:
                    check_field = product.article_id
                    if not check_field:
                        self.context.log.info(f"Product {product.product_name} does not have article_id")
                        continue
                    self.context.log.info(f"Checking if product exists by article_id: {check_field}")
                    result_data = self.api_client.check_product_exists_by_article_id(
                        business_account_id=self.business_account_id,
                        article_id=check_field.strip()
                    )
                
                if result_data.get('exists'):
                    self.context.log.info(f"Product exists, updating retailer_id: {result_data.get('retailer_id')}")
                    
                    update_params = {'retailer_id': result_data.get('retailer_id')}
                    update_needed = False

                    # Check price
                    try:
                        if float(result_data.get('price')) != float(product.price):
                            self.context.log.info(f"Price changed: {result_data.get('price')} → {product.price}")
                            update_params['price'] = product.price
                            update_needed = True
                    except (ValueError, TypeError):
                        self.context.log.info("Price comparison failed")

                    # Check tax
                    tax_percentage = result_data.get('tax_percentage')
                    try:
                        is_tax = float(tax_percentage) > 0.0 if tax_percentage is not None else False
                    except (ValueError, TypeError):
                        is_tax = False
                    ...
                    # Update if needed
                    if update_needed:
                        self.context.log.info("Updating product...")
                        update_result = self.api_client.update_product(**update_params)
                        time.sleep(2)
                        if not update_result.get('success'):
                            self.context.log.error(f"Update failed: {update_result.get('message')}")
                            self.result.errors.append({
                                'stage': 'precheck',
                        ...
                        self.context.log.info("No updates needed")
                else:
                    new_mdb_products.append(product)
                    
            except Exception as e:
                self.context.log.error(f"Precheck failed for product: {str(e)}")
                self.context.log.error(f"{traceback.format_exc()}")
                self.result.errors.append({
                    'stage': 'precheck',
                    'error': str(e),
                    'type': 'warning'
        ...
        self.context.log.info(f"\nPrecheck complete: {len(new_mdb_products)} new products to process")
