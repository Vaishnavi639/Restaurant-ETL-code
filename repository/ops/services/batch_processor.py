...
class ETLBatchProcessor:
    ...
    def process_batch(self, mdb_products: List[MDBProduct]) -> BatchProcessingResult:
        ...
        """
        flow_type = "🍽️ RESTAURANT" if self.is_restaurant else "🛒 GROCERY"
    ...
    def _stage_2_similarity_search(self):
            ...
            try:
                ...
                self.context.log.error(
                    f"Similarity search failed — continuing without matches: {str(e)}",
                    exc_info=True
                )

                # 🔑 IMPORTANT: Treat all products as unmatched
                self.context.log.warning(
                    f"Bypassing similarity search. "
                    f"Proceeding with {len(self.products_to_generate)} products as unmatched."
                )

                # Ensure downstream stages run
                self.similarity_matched_products = []
                self.result.similarity_matched_count = 0
                self.result.similarity_matched_products = []

                self.result.errors.append({
                    'stage': 'similarity_search',
                    'error': str(e),
                    'handled': True,
                    'action': 'bypassed_similarity'
                })
    ...
    def _stage_5_create_master_products(self):
        """Stage 5: Create master products"""
        self.context.log.info(f"\n{'=' * 80}")
        ...
        try:
            master_products_data = [{
                ...
                'vectorEmbedding': p.vector_embedding,
    ...
    def _stage_6_create_final_products(self):
        ...
        Uses NEW batch API format with brand/category/subcategory as name objects
        """
        self.context.log.info(f"\n{'=' * 80}")
        ...
        # ... (rest of method from Document 3, lines 676-837)
